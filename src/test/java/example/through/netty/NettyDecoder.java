...
public class NettyDecoder<T> extends DefaultThrough {
                ...
                switch (status) {
                    ...
                    case StartReadingBody:
                        ...
                        if( buf.remaining() >= contentLength){
                            ...
                            buf.get(tmp);
                            if( dataType == 1 ){
                                String updateString = new String(tmp, StandardCharsets.UTF_8);
                                Update<T> update = gson.fromJson(updateString, updateType);
                    ...
                    case ContinueReadingBody:
                        if( buf.remaining() >= contentLength - bodyBuffer.position()){
                            ...
                            bodyBuffer.get(tmp);
                            if( dataType == 1 ) {
                                String jsonString = new String(tmp, StandardCharsets.UTF_8);
                                try {
                                    Update<T> update = gson.fromJson(jsonString, updateType);
                                    updateBuffer.offer(update);
                                }catch (Throwable e){
                                    throw  e;
                                }
